...
@click.option(
    "--mode",
    type=click.Choice(["duration", "timestamps", "silence"]),
    default="duration",
    show_default=True,
    help="Splitting mode",
)
...
@click.option(
    "--chunk", default=900.0, type=float, help="Chunk duration in seconds (default: 900 = 15 min)"
)
@click.option(
    "--min-silence", default=1000, type=int, help="Minimum silence length in ms (for silence mode)"
)
@click.option("--thresh", default=-40, type=int, help="Silence threshold in dB (for silence mode)")
@click.option(
    "--format",
    "out_format",
    default=None,
...
def cli(input_file, mode, out, chunk, min_silence, thresh, out_format, smart, tolerance):
    ...
    try:
        ...
        if mode == "duration":
            click.echo(
                f"⏱️ Splitting by duration: {chunk}s (smart={smart}, tolerance=±{tolerance}s)..."
            )
            ...
            click.echo(
                "⚠️ Timestamps mode requires a JSON/CSV file. Use --timestamps-file option (coming soon)."
            )
            return
