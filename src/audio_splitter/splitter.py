...
class AudioSplitter:
    ...
    def __init__(
        self, input_path: str, output_dir: str = "output_chunks", output_format: str | None = None
    ):
    ...
    def _detect_silences(self, min_silence_sec: float, thresh_db: int) -> list[tuple[float, float]]:
        cmd = [
            ...
            str(self.input_path),
            "-af",
            f"silencedetect=noise={thresh_db}dB:d={min_silence_sec}",
            "-f",
            "null",
            "-",
        ]
    ...
    def _find_best_split(
        self, target: float, tolerance: float, silences: list[tuple[float, float]]
    ) -> float:
    ...
    def split_by_timestamps(
        self, timestamps: list[tuple[float, float]], prefix: str = "part"
    ) -> list[Path]:
        exported = []
        for i, (start, end) in enumerate(
            tqdm(timestamps, desc=f"⏳ Exporting {prefix}", unit="chunk"), start=1
        ):
    ...
    def split_by_silence(
        ...
        exported = []
        for i, (start, end) in enumerate(
            tqdm(audio_ranges, desc=f"⏳ Exporting {prefix}", unit="chunk"), start=1
        ):
            out_path = self.output_dir / f"{prefix}_{i:03d}.{self.output_format}"
            self._export_chunk(start, end, out_path)
            exported.append(out_path)
